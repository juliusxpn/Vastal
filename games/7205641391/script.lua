...
local can_toggle_auto_player = true -- why
...
local autoplay_toggle = mainTab:Toggle({
	Title = "ignore me lol",
	Type = "Checkbox",
	Default = true,
	Callback = function(v)
...
task.spawn(function()
    while true do
        ...
        if can_toggle_auto_player then
            autoplay_toggle:Set(false)
            autoplay_toggle:Set(true)
        else
            autoplay_toggle:Set(false)
...
local function release_key(keycode)
...
local function play_note(keycode)
    press_key(keycode)
...
-- lol, glad i played expurgation. Fixed it so it doesn't hit bad boy notes (why dont other autoplays have this?)
    ...
    for _, lane in ipairs(key_maps[mode].order) do
        ...
        if not active_holds[lane] then
            for _, n in ipairs(notes:GetChildren()) do
                if n:IsA("GuiObject") and not is_opponent(n) and not n:IsA('Frame') then
...
if mainTab and player then
    ...
    mainTab:Toggle({
        Title = "Disable some GUI'S (This prevents SOME lag? But doesn't prevent those huge massive lagspikes.)", -- bad name actually
        Type = "Checkbox",
        Default = false,
        Callback = function(state)
            hide_enabled = state
